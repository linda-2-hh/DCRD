...
# ================= 配置 =================
test_file = "data/strategyqa/test.json"
local_model_path = "model"
output_file = "test/our_str.json"
knowledge_base = "wikipedia"  # 可选 "wikipedia" 或 "pubmed"
...
# 如果你将来有 PubMed 本地索引，可以写类似函数 retrieve_pubmed(query)

# ================= 推理函数 =================
def ask_model(item):
    q = item["sent1"]
    options = [item["ending0"], item["ending1"]]
    ...
    # ================= 构建 prompt =================
    input_text = f"""You are a careful reasoning assistant.The following are multiple-choice questions. Output a single option as the final answer.
...
Please answer with only the letter of the correct option (A or B).
Answer:"""
